...
Exit: `devtools::load_all()` compiles the whole vendored tree; a smoke
`.Call` runs `MainSolver` on the `examples/test1.cc` problem and returns
`sat`/`unsat` to R.
...
Per the Stage 0 decision. A plausible minimum: `smt_solver()`,
`smt_assert()`, `smt_check()`, `smt_model()`, plus `print`/`format`
methods. Decide error vs. value semantics for `unknown`, and how
...
- ASAN/UBSAN and valgrind runs via `rhub::rhub_check()`; add a
  `--use-valgrind` note to CLAUDE.md.
...
- Verify: `SystemRequirements` accurate, `LICENSE.note` present,
  ...
  the “libs size” note.
- Expect a reviewer question about the bundled third-party sources; have
  the provenance answer ready.
