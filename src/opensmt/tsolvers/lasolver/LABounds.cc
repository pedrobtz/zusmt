...
LABoundStore::printBound(LABoundRef br) const
{
    char *str_out;
    char *v_str_lvr;
    int written = zusmt::alloc_printf(&v_str_lvr, "v%d", ba[br].getLVRef().x);
    assert(written >= 0);
    char* v_str;
    written = zusmt::alloc_printf(&v_str, "%s", v_str_lvr);
    ...
    if ((type == bound_l) && (s == 0))
        written = zusmt::alloc_printf(&str_out, "%s <= %s", r.get_str().c_str(), v_str);
    if ((type == bound_l) && (s != 0))
        written = zusmt::alloc_printf(&str_out, "%s < %s", r.get_str().c_str(), v_str);
    if ((type == bound_u) && (s == 0))
        written = zusmt::alloc_printf(&str_out, "%s <= %s", v_str, r.get_str().c_str());
    if ((type == bound_u) && (s != 0))
        written = zusmt::alloc_printf(&str_out, "%s < %s", v_str, r.get_str().c_str());
...
char* LABoundStore::printBounds(LVRef v) const
{
    ...
    for (unsigned int i = 0; i < var_bounds.size_(); i++) {
        ...
        int written = zusmt::alloc_printf(&tmp, "%s(%s) ", bounds_str, tmp2);
        assert(written >= 0); (void)written;
        free(bounds_str);
        free(tmp2);
        bounds_str = tmp;
