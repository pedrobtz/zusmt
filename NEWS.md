...
* First release. Problems are written in 'SMT-LIB' text; a term-building API is
  planned for a later version.

* Strings crossing to and from the solver now declare UTF-8 rather than
  inheriting the session's encoding. SMT-LIB quoted symbols may be non-ASCII,
  and `|naïve|` previously came back marked native -- correct on a UTF-8
  session and misdecoded on any other.
